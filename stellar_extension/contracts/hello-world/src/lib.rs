...
pub struct Swap {
    ...
    amount: i128,
    hashlock: BytesN<32>,        // ✅ Keep as BytesN<32> for consistency
    timelock: u64,       
    claimed: bool,
}
...
#[contracttype]
pub enum DataKey {
    Swap(BytesN<32>),            // ✅ Keep as BytesN<32>
}
...
impl HTLCContract {
    pub fn initiate(
        ...
        token_address: Address,
        amount: i128,
        hashlock: BytesN<32>,    // ✅ Keep as BytesN<32>
        timelock: u64,
    ) {
        sender.require_auth();
        ...
        // ✅ Fixed: Proper conversion from hash to BytesN<32>
        ...
        env.events().publish(
            ("swap_initiated",), 
            (swap_id_clone, sender_clone, receiver_clone, amount)
        );
    }

    pub fn claim(env: Env, swap_id: BytesN<32>, preimage: Bytes) {
        let swap_id_clone: BytesN<32> = swap_id.clone();
